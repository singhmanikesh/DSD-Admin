...
export function Tournaments() {
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      ...
      setError(null);
      try {
        const res = await fetch("http://187.127.133.215/api/v1/global/tournaments");
  ...
  const performDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`http://187.127.133.215/api/v1/tournaments/${id}`, {
        method: "DELETE",
      });
  ...
  const fetchPlayersForTournament = async (tournamentId) => {
    ...
    setPlayersError(null);
    try {
      const res = await fetch(`http://187.127.133.215/api/v1/tournaments/${tournamentId}/users`);
      if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);
      const data = await res.json();
  ...
  const fetchTeamsForTournament = async (tournamentId) => {
    ...
    setTeamsError(null);
    try {
      const res = await fetch(`http://187.127.133.215/api/v1/tournaments/${tournamentId}/teams`);
      if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);
      const data = await res.json();
